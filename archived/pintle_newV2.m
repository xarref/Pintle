...
dPintle = linspace(dComb/maxChamberToPintleRatio, dComb/minChamberToPintleRatio, 2);
...
%% Pintle configuration and performances
...
for i = 1:length(dFuelHoleOptions)
    ...
    for j = 1:length(rowOptions)
        ...
        for k = 1:length(rPintle)
            ...
            % External radius of the annulus crown and annulus thickness
            total.rOuterOx(k) = sqrt( (aOx)/pi + (rPintle(k)*1e-3)^2 );
            total.annulusThickness(k) = total.rOuterOx(k) - rPintle(k)*1e-3;
            ...
            total.BF(i, j, k) = (total.holesPerRow(i, j)*dFuelHoleOptions(i))/(2*pi*rPintle(k)*1e-3); %this is calculating BF for each row. if the rows are stacked on each other it also is the final bf. if the rows have a gap the resulting Bf will be the sum of the BFs - the overlapping parts.
            ...
            resultsMatrix{i,j,k} = struct(...
                ...
                'rPintle', rPintle(k), ...
                'annulusThickness', total.annulusThickness(k), ...
                'TMR', total.TMR(i, j, k), ...
                'alpha', total.alpha(i, j, k), ...
                'BF', total.BF(i, j, k) ...
                );
        end
    end
...
fprintf('---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n');
            fprintf('| Hole Dia. | Hole Area  | Rows | Holes/Row   | Total Holes  | Mass Flow (kg/s) | Error (%%) || Angular Delta | Arc Distance (mm)  |   rPintle (mm)  |  Ann. Thickness |   TMR   | Alpha (deg) |   BF    |\n');
fprintf('---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n');
for i = 1:length(dFuelHoleOptions)
    for j = 1:length(rowOptions)
        for k = 1:length(rPintle)
            ...
            fprintf('| %.2f mm   | %.4f mm² |   %d  |     %4d    |     %4d     |      %.4f      |   %2.2f%%   ||   %6.2f deg  |    %6.3f mm       |   %6.2f mm     |      %.4f     | %.4f  |   %.3f    |  %.4f |\n', ...
                res.holeDiameter, ...
                res.holeArea, ...
                res.nRows, ...
                res.holesPerRow, ...
...
% for i = 1:length(dFuelHoleOptions)
%     %voOx(i) = dmOx(i)/(rhoo*( (doOx(i)*1e-3/2)^2 - (data.ox.diOx(i)*1e-3/2)^2 )*pi);
