...
aOx = dmOx/(cdAnnulus*sqrt(2*rhoOx*(pInj-pComb)*1e5)); % [m]
% aOx = aOx*1e6; % [mm]

% Fuel density computation
rhoFuel = PropSI('D', 'P', (pInj-6)*1e5, 'T', 490, fuel); %assuming 6 bar pressure drop
aFuel = dmFuel/(cdHole*sqrt(2*rhoFuel*(pInj-pComb)*1e5)); % [m]
% aFuel = aFuel*1e6; % [mm]
...
for i = 1:length(dFuelHoleOptions)
    ...
    nHoles(i) = dmFuel / (cdHole * holeArea(i) * sqrt(2*rhoFuel*(pInj - pComb)*1e5) );
    ...
    for j = 1:length(rowOptions)
        ...
        actualDm(i, j) = cdHole * totalHoles(i, j) * holeArea(i) * sqrt(2*rhoFuel*(pInj - pComb)*1e5);
...
% Hole diameter options (meters)
dOXHoleOptions = [0.3, 0.4, 0.5, 0.6, 0.8] * 1e-3;
rowOptions = [1, 2, 3, 4, 5];
...
for i = 1:length(dOXHoleOptions)
    ...
    nHoles(i) = dmOx / (cdHole * holeArea(i) * sqrt(2*rhoOx*(pInj - pComb)*1e5) );
    maxnHoles(i) = floor(nHoles(i));
    ...
    for j = 1:length(rowOptions)
        ...
        holesPerRows(i, j) = floor(nHoles(i) / rowOptions(j));
        totalHoles(i, j) = holesPerRows(i, j) * rowOptions(j);
        actualDm(i, j) = cdHole * totalHoles(i, j) * holeArea(i) * sqrt(2*rhoOx*(pInj - pComb)*1e5);
...
for k = 1:length(rPintle)
            %arcDistance(i, j, k) = ((rPintle(k)*1e-3) * angularSpacingRad(i, j))*1e3;
            rPintlef = rPintle(k);
            rOuterFuel = sqrt( (aOx)/pi + (rPintlef*1e-3)^2 );
            rOuterOxf(k) = rOuterFuel;
