...
for configuration = 1:2
    ...
    rhoFuel = PropSI('D', 'P', (SET.pInj-6)*1e5, 'T', 490, SET.fuel); 
...
dFuelHoleOptions = [0.3, 0.4, 0.5, 0.6] * 1e-3;
rowOptions = [1, 2, 3, 4];

% Prepare storage for results
resultsMatrix = cell(length(dFuelHoleOptions), length(rowOptions));
...
% Iterate through hole diameters
    ...
    for i = 1:length(dFuelHoleOptions)
        ...
        % Compute max holes needed to match mass flow
        maxHoles = ceil(SET.dmFuel / (SET.cdFuelHole * holeArea * sqrt(2*rhoFuel*(SET.pInj - SET.pComb)*1e5)));
        ...
        for j = 1:length(rowOptions)
            ...
            holesPerRow = ceil(maxHoles / nRows);
            angularSpacing = 360 / (holesPerRow);
            
            % Actual total holes used
            totalHoles = holesPerRow * nRows;
            
            % Compute actual mass flow rate
            actualMassFlow = SET.cdFuelHole * totalHoles * holeArea * sqrt(2*rhoFuel*(SET.pInj - SET.pComb)*1e5);
            
            % Compute percent error from target mass flow
            percentError = abs(actualMassFlow - SET.dmFuel) / SET.dmFuel * 100;
            
            % Store results
            resultsMatrix{i,j} = struct(...
...
% Compute external oxidizer radius for each rPost
for i = 1:length(rPost)
    rExtOx(i) = sqrt(aOx/pi + rPost(i)^2);
end

% Display results in a table format
fprintf('Fuel Discrete Hole Configuration Analysis\n\n');
fprintf('---------------------------------------------------------------------------------------------------------\n');
fprintf('| Hole Dia. | Hole Area  | Rows | Holes/Row | Total Holes | Mass Flow (kg/s) | Error (%%) || Angular Delta |\n');
fprintf('---------------------------------------------------------------------------------------------------------\n');

for i = 1:length(dFuelHoleOptions)
    for j = 1:length(rowOptions)
        res = resultsMatrix{i,j};
        
        for k = 1:length(rPost)
            fprintf('| %.2f mm   | %.4f mm² |   %d  |     %d    |     %d     |      %.4f      |   %.2f%%  ||   %.3f deg  |\n', ...
                res.holeDiameter, ...
                res.holeArea, ...
                res.nRows, ...
                res.holesPerRow, ...
...
% Display Oxidizer Annulus Information
fprintf('\nOxidizer Annulus Information:\n');
fprintf('---------------------------------------------\n');
fprintf('| rPost (mm) | Oxidizer Area (mm²) | Annulus Width (mm) |\n');
fprintf('---------------------------------------------\n');
for i = 1:length(rPost)
    fprintf('| %.2f     | %.4f           | %.4f           |\n', ...
        rPost(i), ...
